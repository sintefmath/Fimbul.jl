...
"""
    ht_ates(; <keyword arguments>)

Create a High-Temperature Aquifer Thermal Energy Storage (HT-ATES) simulation
case using the `H2OSystem` two-phase water model (pressure-enthalpy formulation).

Based on the `ates()` setup function, extended to support high-temperature
operation. The `H2OSystem` PVT framework uses steam tables to compute accurate
fluid properties at elevated temperatures and handles potential two-phase
(liquid/vapour) conditions.

Injection and boundary enthalpies are derived from the steam tables to be
...
## Geometry Parameters
- `well_distance = missing`: Distance between hot and cold wells [m]. If missing,
  calculated as 4×thermal_radius
- `depths = [0.0, 450.0, 500.0, 600.0, 650.0, 900.0]`: Layer interface depths [m].
  By default the aquifer starts at 500 m depth.
- `aquifer_layer = 3`: Index of the aquifer layer for thermal storage
- `use_2d = false`: If true, creates a 2D model
...
## Operational Parameters
- `temperature_charge = 200°C`: Temperature of injected water during charging [K]
- `temperature_discharge = 50°C`: Temperature of injected water during discharging [K]
- `rate_charge = 100 l/s`: Injection/production rate during charging [m³/s]
- `rate_discharge = rate_charge`: Rate during discharging [m³/s]
- `balanced_injection = true`: If true, the injection well target will be set
  equal to the production rate
...
# Returns
A `JutulCase` for HT-ATES using `H2OSystem`
"""
function ht_ates(;
    well_distance = missing,
    depths = [0.0, 450.0, 500.0, 600.0, 650.0, 900.0],
    porosity = [0.01, 0.05, 0.35, 0.05, 0.01],
    permeability = [1.0, 5.0, 1000.0, 5.0, 1.0].*1e-3.*si_unit(:darcy),
    rock_thermal_conductivity = [2.5, 2.0, 1.5, 2.0, 2.5].*watt/(meter*Kelvin),
    rock_heat_capacity = 900.0*joule/(kilogram*Kelvin),
    rock_density = fill(2600.0*kilogram/meter^3, length(depths)-1),
    aquifer_layer = 3,
    temperature_charge = convert_to_si(200, :Celsius),
    temperature_discharge = convert_to_si(50, :Celsius),
    rate_charge = 100*litre/second,
    rate_discharge = rate_charge,
    balanced_injection = true,
    temperature_surface = convert_to_si(10.0, :Celsius),
    thermal_gradient = 0.03Kelvin/meter,
    ...
    # ## Initialize H2OSystem and load steam tables
    sys = H2OSystem()
    ...
    Caq = Cf*ϕ + Cr*(1 - ϕ)
    Haq = layer_thickness[aquifer_layer]
    # Default charging duration (June-September)
    ch_start = Fimbul.process_time(2025, charge_period[1])
    ch_end = Fimbul.process_time(2025, charge_period[2], true)
    charge_duration = (ch_end - ch_start).value*1e-3
    thermal_radius = missing
    # Calculate rate based on a target thermal radius if not provided
    if ismissing(rate_charge)
        thermal_radius = ismissing(well_distance) ? 125 : well_distance/4
        rate_charge = (thermal_radius^2*Caq*π*Haq/Cf)/charge_duration
        # Adjust for 2D simulation (per unit width)
    ...
    Vin = rate_charge*charge_duration
    # Calculate thermal radius from injected volume if not already set
    if ismissing(thermal_radius)
        thermal_radius = thermal_radius_aquifer(Vin, Haq, ϕ, Cf, Cr)
    end
    # Set well distance based on thermal radius if not specified
    ...
    # ## Setup reservoir model with H2OSystem
    ...
    dpdz = rho_ref*gravity_constant
    dTdz = thermal_gradient
    p_surface = 10atm
    p = z -> p_surface .+ dpdz.*z
    T = z -> temperature_surface .+ dTdz*z
    # Set boundary conditions
    z_bdr = geo.boundary_centroids[3, :]
    xy_bdr = geo.boundary_centroids[1:2, :]
    ...
    z_bc = z_bdr[subset]
    z_hat_bc = z_bc .- z0
    # Compute BC pressure, temperature, and consistent enthalpy from steam tables
    p_bc = p(z_hat_bc)
    T_bc = T(z_hat_bc)
    h_bc = tables[:enthalpy].(p_bc, T_bc)
    bc = flow_boundary_condition(cells_bc, rmodel.data_domain, p_bc, T_bc; enthalpy = h_bc)
    # Setup initial state with hydrostatic pressure, geothermal temperature, and
    # consistent enthalpy from the steam tables
    z_hat = geo.cell_centroids[3, :] .- z0
    p0 = p(z_hat)
    T0 = T(z_hat)
    ...
    # ## Compute injection enthalpies and densities at aquifer conditions
    # Use hydrostatic pressure at the aquifer midpoint depth for enthalpy lookups
    z_aq = (depths[aquifer_layer] + depths[aquifer_layer + 1]) / 2 - z0
    p_aq = p(z_aq)
    h_charge   = tables[:enthalpy](p_aq, temperature_charge)
    h_discharge = tables[:enthalpy](p_aq, temperature_discharge)
    rho_charge   = tables[:density_mix](p_aq, h_charge)
    rho_discharge = tables[:density_mix](p_aq, h_discharge)
    ...
    ctrl_cold = InjectorControl(
        rate_target, [1.0];
        density = rho_discharge,
        temperature = temperature_discharge,
        enthalpy = h_discharge,
        check = false,
    )
    # Set up forces
    control = Dict(:Hot => ctrl_hot, :Cold => ctrl_cold)
    ...
    # ## Additional info
    info = Dict()
    info[:description] = "High-Temperature Aquifer Thermal Energy Storage (HT-ATES) case "*
        "generated using Fimbul.ht_ates() with H2OSystem"
    info[:well_distance] = well_distance
    info[:thermal_radius] = thermal_radius
    info[:layers] = layers
    info[:aquifer_layer] = aquifer_layer
    ...
    # ## Create and return the complete simulation case
    case = JutulCase(model, dt, forces; state0 = state0, parameters = parameters, input_data = info)
    return case
