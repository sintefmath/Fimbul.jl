function extruded_mesh(cell_constraints::Vector{<:AbstractMatrix}, depths;
    ...
    total_points = sum(size(cc, 1) for cc in cell_constraints)
    x_cc = Matrix{Float64}(undef, total_points, 2)
    ...
    for (i, cc) in enumerate(cell_constraints)
        ...
        if n_points == 1
            ...
            t0d_i = add_geometry_0d(cc[1:1, 1:2], hxy_min, i0, true)  # Extract 2D coordinates
            push!(tag0d_cc, t0d_i...)
            i0 += 1
        else
