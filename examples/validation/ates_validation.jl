...
function analytical_temperature_timeseries(case, r_real, α_L)
    domain = reservoir_model(case.model).data_domain
    t = DAYS .* si"day"
    Q = case.input_data[:injection_rate]
    T_i = convert_from_si(case.input_data[:injection_temperature], :Celsius)
    T_0 = convert_from_si(case.input_data[:boundary_temperature], :Celsius)
    ρ_f = 998.0
    b = last(case.input_data[:layer_depths]) - first(case.input_data[:layer_depths])
    return Fimbul.radial_analytical(r_real, t, Q, T_i, T_0, ρ_f, α_L, b, domain)
end
...
function plot_radial_time_comparison(case, results; α_L = 0.0, plot_radii = 1:5:40, title = "Radial benchmark")
    ...
    for (k, r_key) in enumerate(plot_radii)
        ...
        r_real = r_map[r]
        T_a = analytical_temperature_timeseries(case, r_real, α_L)
...
function estimate_cfl_timestep(case; target_cfl = 0.9)
    data = case.input_data
    radii = data[:radii]
    layer_depths = data[:layer_depths]
    injection_rate = data[:injection_rate]
    porosity = get(data, :porosity, 0.35)

    length(radii) >= 2 || error("Need at least two radial samples to estimate CFL timestep.")
    height = last(layer_depths) - first(layer_depths)
    Δr = radii[2] - radii[1]
    r_ref = 10.0
    v_ref = injection_rate/(2*pi*r_ref*height*porosity)
    return target_cfl*Δr/v_ref
end
...
function compute_case_error(case, results; α_L = 0.0, plot_radii = 5:5:40)
    ...
    for r_key in plot_radii
        ...
        T_a = analytical_temperature_timeseries(case, r_map[r], α_L)
        push!(errs, sqrt(mean((T_sim[r] .- T_a).^2)))
    end
    return mean(errs)
end

function convergence_study(; with_dispersion = false, nrad_values = [100, 200, 400, 800], nang = 4, α_L = 1.0, α_T = 0.1)
    rows = DataFrame(
        nrad = Int[],
        n_cells = Int[],
        h = Float64[],
    ...
    for nrad in nrad_values
        thermal_dispersivity = with_dispersion ? permutedims([α_L α_T]) : nothing
        case = Fimbul.analytical_radial(
            thermal_dispersivity = thermal_dispersivity,
            nrad = nrad,
            nang = nang,
            layer_depths = [0.0, 20.0],
        )
        max_timestep = 0.5*estimate_cfl_timestep(case)
...
function plot_convergence(rows; title = "Convergence")
    ...
    lines!(ax, h0, h0.^0.5, linewidth = 6, linestyle = :dash, color = :coral, label = "1/2-order reference")
    lines!(ax, h0, h0, linewidth = 6, linestyle = :dash, color = :skyblue3, label = "1st-order reference")
...
sim_no_disp, cfg_no_disp = setup_reservoir_simulator(
    case_no_disp;
    initial_dt = 1.0,
    info_level = 0,
...
sim_disp, cfg_disp = setup_reservoir_simulator(
    case_disp;
    initial_dt = 1.0,
    info_level = 0,
    max_timestep = 0.1*si"day",
